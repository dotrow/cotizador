...
this.showAlbums = function(root){
	...
	var pwaImageSize = 160; // default image size
	...
	for ( i = 0, r = 0, c = 0; i < pwaFetch ; ++i){
		...
		vinculo.setAttribute( "onclick", "gallery.loadscript('" + entry.id.$t +"')" ); 
...
this.showAlbum = function( root, page ){
    ...
    // Fin del html
		...
		/*td.innerHTML= "<a href=\""+url+"\" class='thickbox' rel='gallery-album' title='"++"'><img class='wp_img' src='"+ url2 + "'></a>";
		td.innerHTML+= "<div class='wp_title'><a href='"+link+"'>" + title + "</a></div>";
		td.innerHTML+= "<div class='wp_date'>" + entry.updated.$t + "</div>";*/
	...
	div.innerHTML = div.innerHTML;
	div.innerHTML = backtoAlbums()+this.Pagination( pwaFetch, root ) + div.innerHTML;
	div.innerHTML += this.Pagination( pwaFetch, root )+backtoAlbums();
...
this.loadscript = function ( source ){
  // show loading message
	...
	script.src = source.replace( "entry", "feed" ) + "&callback=gallery.showAlbum&kind=photo";
...
function backtoAlbums(){
	return "<div align='center'><a class='linkw' onclick='gallery.showAlbums();'>Ver albums</a></div>"
};
